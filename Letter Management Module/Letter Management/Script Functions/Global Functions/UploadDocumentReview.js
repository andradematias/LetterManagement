...
var showRemoveButton = Options?.showRemoveButton;
var showValidInvalidButtons = Options?.showValidInvalidButtons;
var showVerifyStatus = Options?.showVerifyStatus;
...
if (targetFormID == 'self' && VV.Form.IsFormSaved == false) {
    var modalTitle = 'Save to View Documents';
    var modalBody = 'Please "Save" the form in order to view the uploaded documents.';
...
var CallServerSide = function () {
    ...
    var FormInfo = {};
    FormInfo.name = 'Target Form ID';
    FormInfo.value = targetFormID;
    formData.push(FormInfo);
    ...
    if (typeof (resp.status) != 'undefined') {
        ...
        if (resp.data[0] != 'undefined') {
            if (resp.data[0] == 'Success') {
                ...
                var documentsFound = resp.data[2];
                var instructions = 'Review uploaded documents by clicking the "Open" button. ';
                if (documentsFound.length < 1) {
                    instructions = 'No documents were found related to the current form. Please upload a document to see it in this list.';
                } else if (showValidInvalidButtons) {
                    instructions = 'Please verify the related documents by clicking the corresponding "Valid" or "Invalid" button. ';
                } else if (showRemoveButton) {
                    instructions += 'Remove any unwanted documents by clicking the "Remove File" button. ';
                }
                $('#documentMessageArea').append($(`<p class="documentUserMessage">${instructions}</p>`));

                // Add styling so scroll bar shows on overflow
                var mainContent = $('#documentButtonArea');
                mainContent.css({
                    'max-height': '512px',
                    'overflow-y': 'auto',
                });
                ...
                // Build table
                var documentsTable = $('<table style="width:100%;">');
                documentsFound.forEach(function (doc) {
                    var docID = doc.documentId.trim();
                    var docGUID = doc.id.trim();
                    var docURL = VV.BaseURL + 'documentviewer?&hidemenu=true&dhid=' + docGUID;
                    var docName = doc.description.trim();
                    docName = docName ? docName : 'Unknown';
                    var docUploadDate = doc.createDate.split('T')[0];
                    docUploadDate = docUploadDate ? docUploadDate : 'Unknown';
                    var docType = doc['checklist task document type']?.trim();
                    docType = docType ? docType : 'Unknown';
                    var padbot = '18px'; // spacing between each row

                    /* verify status */
                    var verifTextID = "verif" + docID;
                    var verifColor = "#f70606";
                    var verifLabel = "Unverified";
                    var validBtnID = "valid" + docID;
                    var validBtnDisabled = "";
                    var validBtnOpacity = "1";
